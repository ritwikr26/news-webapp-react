  ...
  render() {
    ...
    if (!loading) {
      ...
      return (
        ...
        <button className="btn btn-outline-success" onClick={this.searchquery} type="button">Search News</button>
      </form>
            <div className="row">
              {this.state.articles.map((e) => {
                return <><div className="col" key={e.url}>
                  <Newsitems title={e.title} description={e.description} url={e.url} imgurl={e.urlToImage} ndate={e.publishedAt} />
                </div></>
              })}
            </div>
